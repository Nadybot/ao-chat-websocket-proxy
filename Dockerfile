...
RUN rm -f target/$RUST_TARGET/release/deps/ao_chat_websocket_proxy*
COPY ./src ./src

RUN source $HOME/.cargo/env && \
    cargo build --release \
        --target="$RUST_TARGET" && \
    cp target/$RUST_TARGET/release/ao-chat-websocket-proxy /ao-chat-websocket-proxy && \
    actual-strip /ao-chat-websocket-proxy

FROM scratch

COPY --from=builder /ao-chat-websocket-proxy /ao-chat-websocket-proxy

CMD ["./ao-chat-websocket-proxy"]
